...
function Hockey(){
    ...
    const [nhlPlayer, setNhlPlayer]= useState([])
    ...
    useEffect(() => {
        fetch("http://localhost:9292/player/nhl")
          .then((r) => r.json())
          .then((players) => setNhlPlayers(players));
      }, []);
      ...
      function handleSubmit(event){
        ...
        fetch("http://localhost:9292/testplayer/nhl", {
          method: "POST",
          headers: {
        ...
        .then((r) => r.json())
        .then((newPlayer) => setNhlPlayers(newPlayer));
        setAddPlayer({
            playerName: '',
            playerTeam: '',
          })
    ...
    showPlayerCards(player)
        ...
        <button type="submit">Add Player</button>
            ...
            <section className="cards">
                <ul>
                    {nhlPlayers.map((player) => (
                    <PlayerItem key={player.id} player={player} showPlayerCards={showPlayerCards}/> 
                    ))}
                </ul>
            <TestSample cards={cards} player={nhlPlayer} handleUpdate={handleUpdate} handleDelete={handleDelete}/>
            <Form player={nhlPlayer} setCards={setCards} updateInfo={updateInfo} tryChange={tryChange} clearUpdateCard={clearUpdateCard}/>
            </section>
        </div>
    )
}
