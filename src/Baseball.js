...
function Baseball(){
  ...
  const [mlbPlayer, setMlbPlayer]= useState([])
  ...
  const [addPlayer, setAddPlayer] = useState({
      ...
      useEffect(() => {
        fetch("http://localhost:9292/player/mlb")
          .then((r) => r.json())
          .then((players) => setMlbPlayers(players));
      }, []);
      ...
      function handleSubmit(event){
        ...
        fetch("http://localhost:9292/testplayer/mlb", {
          method: "POST",
          headers: {
        ...
        .then((r) => r.json())
        .then((newPlayer) => setMlbPlayers(newPlayer));
        setAddPlayer({
            playerName: '',
            playerTeam: '',
          })
  ...
  function doDelete(deletedReview, player){
    ...
    return (
      ...
      <button type="submit">Add Player</button>
          ...
          <TestSample cards={cards} player={mlbPlayer} handleUpdate={handleUpdate} handleDelete={handleDelete}/>
          <Form player={mlbPlayer} setCards={setCards} updateInfo={updateInfo} tryChange={tryChange} clearUpdateCard={clearUpdateCard}/>
          </section>
      </div>
)
}
